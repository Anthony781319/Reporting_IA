...
export default function DashboardRH() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      let repQuery = supabase.from('cr_reporting').select('*').eq('annee', annee)
      let rdvQuery = supabase.from('cr_rendez_vous').select('*').eq('annee', annee)
      let presQuery = supabase.from('cr_presentations').select('*').eq('annee', annee)
      let sigQuery = supabase.from('cr_signatures').select('*').eq('annee', annee)

      if (mode === 'semaine') {
        repQuery = repQuery.eq('semaine', semaine)
        rdvQuery = rdvQuery.eq('semaine', semaine)
        presQuery = presQuery.eq('semaine', semaine)
        sigQuery = sigQuery.eq('semaine', semaine)
      }

      const [{ data: rep }, { data: rdv }, { data: pr }, { data: sig }] = await Promise.all([
        repQuery, rdvQuery, presQuery, sigQuery
      ])
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px' }}>
      ...
      {mode === 'semaine' && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 20 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, minWidth: 100, textAlign: 'center' }}>Semaine {semaine}</span>
